...
$.ajax({
  ...
  data: {
    key: '4ANBZ-O5WC4-KQ3UD-K2D3Q-4APEE-DMFF2',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 66.6 + 'px',
        nekoImg: "https://tuchuang.voooe.cn/images/2024/04/13/cat.png",
        hoverMsg: "春天啦~",
...
function createtime1() {
  var grt = new Date("10/30/2022 00:00:00"); //此处修改你的建站时间或者网站上线时间
  ...
  var ascll = [
    `欢迎来到打工人wlog!`,
    `打工人wlog,wlog全称work blog,记录新时代年轻打工人在外漂泊的辛酸苦辣🍭🍭🍭`,
    `
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Fomalhaut🥝 %c 你正在访问打工人wlog",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 打工人wlog") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('打工人wlog的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("10/30/2022 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("10/30/2022 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://pic.wlog.online/icon/打工人-科学摸鱼中.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://pic.wlog.online/icon/打工人-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://pic.wlog.online/images/home_bg.webp"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
